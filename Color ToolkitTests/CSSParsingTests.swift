...
struct CSSParseRejectionTests {
  ...
  func unsupportedFunctions() {
    // The naming matters: a `calc()` body contains operators the color tokenizer
    // has no rules for, so without an early check the user would see
    // "unexpected character +" instead of what is actually wrong.
    #expect(throws: ParseError.unsupportedFunction("calc")) {
      try CSSColorParser.parse("rgb(calc(1 + 1) 0 0)")
    }
    ...
    expectRejected("oklch(clamp(0, 0.5, 1) 0.1 200)")
