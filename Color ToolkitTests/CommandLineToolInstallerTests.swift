...
/// - Note: `Color ToolkitTests` carries no `ENABLE_APP_SANDBOX` of its own — only the
...
struct CommandLineToolInstallerTests {
  ...
  @Test("The embedded binary sits in Contents/MacOS, not Contents/Executables")
  func embeddedBinaryURLPointsAtContentsMacOS() {
    let bundle = URL(fileURLWithPath: "/Applications/Color Toolkit.app")
    let result = CommandLineToolInstaller.embeddedBinaryURL(inBundleAt: bundle)
    #expect(result.path == "/Applications/Color Toolkit.app/Contents/MacOS/colorkit")
  }
  ...
  func isTranslocatedDetectsOnlyTheQuarantineCopy() {
    #expect(!CommandLineToolInstaller.isTranslocated(
      bundlePath: "/Applications/Color Toolkit.app/Contents/MacOS/colorkit",
    ))
    #expect(CommandLineToolInstaller.isTranslocated(
      bundlePath: "/private/var/folders/xy/T/AppTranslocation/1234-5678/d/"
        + "Color Toolkit.app/Contents/MacOS/colorkit",
    ))
  }
  ...
  func installRefusesATranslocatedBundle() {
    let translocatedBinary = URL(fileURLWithPath:
      "/private/var/folders/xy/T/AppTranslocation/1234-5678/d/Color Toolkit.app/Contents/MacOS/colorkit")
