...
struct ExitCodeTests {
  ...
  func usageAndFailureAreSeparate() {
    ...
    #expect(ColorToolkitCLI.run(["convert", "red", "--nope"]).status == .usage)
    #expect(ColorToolkitCLI.run(["convert", "notacolor"]).status == .failure)
    #expect(ColorToolkitCLI.run(["convert", "red", "--format", "hex"]).status == .success)
  }
  ...
  func streamsAreSeparate() {
    ...
    let failed = ColorToolkitCLI.run(["convert", "notacolor"])
    #expect(failed.output.isEmpty)
    #expect(!failed.diagnostic.isEmpty)

    let misused = ColorToolkitCLI.run(["convert"])
  ...
  func warningsDoNotContaminateTheResult() {
    ...
    let outcome = ColorToolkitCLI.run(["convert", "oklch(0.7, 0.1, 250)", "--format", "hex"])
  ...
  func helpAndEmptinessDiffer() {
    ...
    let empty = ColorToolkitCLI.run([])
    #expect(empty.status == .usage)
    #expect(empty.output.isEmpty)

    let help = ColorToolkitCLI.run(["--help"])
  ...
  func unknownCommandIsRejected() {
    let outcome = ColorToolkitCLI.run(["bogus"])
