...
//  `Color ToolkitTests` against colorjs.io and against their own defining properties.
...
struct DispatchTests {
  ...
  func everyCommandIsReachable() {
    // The listing in `ColorToolkitCLI.commands` is what `--help` prints, and a command
    ...
    for command in ColorToolkitCLI.commands {
      let outcome = ColorToolkitCLI.run([command.name, "--help"])
  ...
  func helpDoesNotHideAUsageError() {
    ...
    #expect(ColorToolkitCLI.run(["ramp", "red", "--help"]).status == .success)
    #expect(ColorToolkitCLI.run(["ramp", "--stops", "--help"]).status == .usage)
  }

  @Test("Too few and too many positionals are both usage errors")
  func operandCountIsChecked() {
    #expect(ColorToolkitCLI.run(["convert"]).status == .usage)
    #expect(ColorToolkitCLI.run(["convert", "red", "blue"]).status == .usage)
    #expect(ColorToolkitCLI.run(["contrast", "red"]).status == .usage)
    #expect(ColorToolkitCLI.run(["contrast", "red", "white", "blue"]).status == .usage)
  }
}
...
struct ConvertCommandTests {
  ...
  func singleFormatOutputIsBare() {
    ...
    let outcome = ColorToolkitCLI.run(["convert", "#3b82f6", "--format", "oklch"])
  ...
  func everyListedFormatSurvivesTheParser() {
    let outcome = ColorToolkitCLI.run(["convert", "#3b82f6"])
  ...
  func keywordIsSkippedWhenThereIsNone() {
    ...
    let listed = ColorToolkitCLI.run(["convert", "#3b82f6"])
    #expect(!listed.output.contains("keyword"))

    let asked = ColorToolkitCLI.run(["convert", "#3b82f6", "--format", "keyword"])
    #expect(asked.status == .failure)
    #expect(asked.output.isEmpty)

    let exists = ColorToolkitCLI.run(["convert", "#663399", "--format", "keyword"])
  ...
  func gamutMappingIsReportedTwoWays() {
    ...
    let table = ColorToolkitCLI.run(["convert", wide])
    ...
    let single = ColorToolkitCLI.run(["convert", wide, "--format", "hex"])
    #expect(!single.output.contains("gamut"))
    #expect(single.diagnostic.contains("brought into gamut"))

    let unbounded = ColorToolkitCLI.run(["convert", wide, "--format", "oklch"])
  ...
  func theParserIsTheMixCommand() {
    ...
    let mixed = ColorToolkitCLI.run(["convert", "color-mix(in oklch, red, blue)",
                                     "--format", "oklch"])
    #expect(mixed.status == .success)
    #expect(CSSColorParser.color(mixed.output) != nil)

    let relative = ColorToolkitCLI.run(["convert", "rgb(from red calc(r / 2) g b)",
                                        "--format", "rgb"])
  ...
  func precisionIsHonouredAndBounded() {
    let coarse = ColorToolkitCLI.run(["convert", "#3b82f6", "--format", "oklch",
                                      "--precision", "2"])
    let fine = ColorToolkitCLI.run(["convert", "#3b82f6", "--format", "oklch",
                                    "--precision", "8"])
    #expect(coarse.output != fine.output)
    #expect(coarse.output.count < fine.output.count)
    #expect(ColorToolkitCLI.run(["convert", "red", "--precision", "11"]).status == .usage)
  }
}
...
struct ContrastCommandTests {
  ...
  func theReportIsComplete() {
    let outcome = ColorToolkitCLI.run(["contrast", "#333333", "#ffffff"])
    #expect(outcome.status == .success)
  ...
  func aReportIsNotAVerdict() {
    ...
    let failing = ColorToolkitCLI.run(["contrast", "#777777", "#888888"])
  ...
  func requireDecidesTheExitCode() {
    let fails = ColorToolkitCLI.run(["contrast", "#777777", "#888888", "--require", "aa"])
    ...
    let passes = ColorToolkitCLI.run(["contrast", "#333333", "#ffffff", "--require", "aaa"])
    #expect(passes.status == .success)

    #expect(ColorToolkitCLI.run(["contrast", "red", "white", "--require", "AA"]).status == .usage)
  }
  ...
  func solvedColorsMeetTheirTarget() throws {
    ...
    for level in ["aa", "aa-large", "aaa"] {
      let outcome = ColorToolkitCLI.run(["solve", "#3b82f6", "--on", "#ffffff",
                                         "--level", level])
      #expect(outcome.status == .success)
  ...
  func roundingAwayTheMarginIsReported() throws {
    ...
    let coarse = ColorToolkitCLI.run(["solve", "#3b82f6", "--on", "#ffffff",
    ...
    let fine = ColorToolkitCLI.run(["solve", "#3b82f6", "--on", "#ffffff",
  ...
  func unreachableTargetsAreExplained() {
    ...
    let outcome = ColorToolkitCLI.run(["solve", "#3b82f6", "--on", "#767676",
                                       "--level", "aaa"])
  ...
  func solveRejectsAmbiguousArguments() {
    #expect(ColorToolkitCLI.run(["solve", "red"]).status == .usage)
    #expect(ColorToolkitCLI.run(["solve", "red", "--on", "white",
...
struct TransformCommandTests {
  ...
  func harmoniesAreCompleteAndReadable() {
    for harmony in Harmony.allCases {
      let outcome = ColorToolkitCLI.run(["harmony", "#3b82f6", harmony.rawValue])
  ...
  func achromaticHarmoniesAreExplained() {
    let gray = ColorToolkitCLI.run(["harmony", "#808080", "triad"])
    ...
    #expect(ColorToolkitCLI.run(["harmony", "#808080", "monochromatic"]).diagnostic.isEmpty)
  }

  @Test("An option that only applies to one harmony is refused for the others")
  func inapplicableHarmonyOptionsAreRefused() {
    #expect(ColorToolkitCLI.run(["harmony", "red", "analogous", "--spread", "40"])
      .status == .success)
    #expect(ColorToolkitCLI.run(["harmony", "red", "triad", "--spread", "40"])
      .status == .usage)
    #expect(ColorToolkitCLI.run(["harmony", "red", "monochromatic", "--stops", "7"])
      .status == .success)
    #expect(ColorToolkitCLI.run(["harmony", "red", "triad", "--stops", "7"])
      .status == .usage)
  }
  ...
  func rampStopsAreInGamut() throws {
    ...
    let outcome = ColorToolkitCLI.run(["ramp", "#3b82f6"])
    #expect(outcome.status == .success)
    let lines = outcome.output.split(separator: "\n").map(String.init)
  ...
  func evenStopCountsAreRoundedUp() {
    #expect(ColorToolkitCLI.run(["ramp", "red", "--stops", "10"])
      .output.split(separator: "\n").count == 11)
    #expect(ColorToolkitCLI.run(["ramp", "red", "--stops", "2"]).status == .usage)
    #expect(ColorToolkitCLI.run(["ramp", "red", "--lightest", "0.2", "--darkest", "0.9"])
      .status == .usage)
  }
  ...
  func gamutWidensTheRamp() throws {
    ...
    let narrow = ColorToolkitCLI.run(["ramp", "#3b82f6", "--gamut", "srgb"])
    let wide = ColorToolkitCLI.run(["ramp", "#3b82f6", "--gamut", "rec2020"])
    #expect(narrow.output != wide.output)
    #expect(ColorToolkitCLI.run(["ramp", "red", "--gamut", "cmyk"]).status == .usage)
  ...
  func cvdCoversEveryDeficiency() {
    let all = ColorToolkitCLI.run(["cvd", "#e11d48"])
    ...
    let one = ColorToolkitCLI.run(["cvd", "#e11d48", "--type", "protanomaly"])
    #expect(one.output.split(separator: "\n").count == 1)
    #expect(ColorToolkitCLI.run(["cvd", "red", "--type", "monochromacy"]).status == .usage)
  }
  ...
  func severityIsHonoured() {
    ...
    let none = ColorToolkitCLI.run(["cvd", "#e11d48", "--type", "deuteranomaly",
                                    "--severity", "0"])
    let full = ColorToolkitCLI.run(["cvd", "#e11d48", "--type", "deuteranomaly",
                                    "--severity", "1"])
    #expect(none.output != full.output)
    #expect(ColorToolkitCLI.run(["cvd", "red", "--severity", "2"]).status == .usage)
  }
}
