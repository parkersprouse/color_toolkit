...
struct OutputOptionTests {
  ...
  func everyShapeSurvivesTheParser() {
    ...
    for shape in ExportShape.allCases {
      ...
      for command in [["ramp", "#3b82f6"], ["harmony", "#3b82f6", "complementary"]] {
        let outcome = ColorToolkitCLI.run(command + ["--shape", name])
  ...
  func inertOptionsAreRefused() {
    ...
    #expect(ColorToolkitCLI.run(["ramp", "red", "--shape", "p3-with-fallback",
    ...
    #expect(ColorToolkitCLI.run(["ramp", "red", "--shape", "declaration",
                                 "--name", "brand"]).status == .usage)
    #expect(ColorToolkitCLI.run(["ramp", "red", "--shape", "declaration",
                                 "--template", "border"]).status == .success)
    #expect(ColorToolkitCLI.run(["ramp", "red", "--shape", "custom-properties",
                                 "--template", "border"]).status == .usage)
    #expect(ColorToolkitCLI.run(["ramp", "red", "--shape", "custom-properties",
                                 "--name", "brand"]).status == .success)

    // Without a shape there is no document at all, so both are inert.
    #expect(ColorToolkitCLI.run(["ramp", "red", "--name", "brand"]).status == .usage)
    #expect(ColorToolkitCLI.run(["ramp", "red", "--template", "border"]).status == .usage)
  }

  @Test("--name reaches the document it names")
  func nameReachesTheOutput() {
    let outcome = ColorToolkitCLI.run(["ramp", "#3b82f6", "--shape", "custom-properties",
                                       "--name", "accent"])
    #expect(outcome.output.contains("--accent-50:"))
    #expect(!outcome.output.contains("--brand-"))
    #expect(ColorToolkitCLI.run(["ramp", "red", "--shape", "json", "--name", " "])
      .status == .usage)
  }
  ...
  func theMappedNoteFollowsTheFormat() {
    ...
    let hex = ColorToolkitCLI.run(["harmony", "color(display-p3 0 1 0)", "triad",
                                   "--format", "hex"])
    #expect(hex.diagnostic.contains("brought into gamut"))
    #expect(hex.diagnostic.contains("hex"))

    let oklch = ColorToolkitCLI.run(["harmony", "color(display-p3 0 1 0)", "triad",
                                     "--format", "oklch"])
  ...
  func theP3NoteCountsTheFallback() {
    ...
    let outcome = ColorToolkitCLI.run(["harmony", "color(display-p3 0 1 0)", "triad",
...
struct ExportCommandTests {
  ...
  func keysComeFromTheCommandLine() {
    let positional = ColorToolkitCLI.run(["export", "red", "blue"])
    #expect(positional.output.contains("--brand-1:"))
    #expect(positional.output.contains("--brand-2:"))

    let named = ColorToolkitCLI.run(["export", "primary=red", "secondary=blue"])
  ...
  func keysAreUniquedAfterSanitizing() {
    ...
    let collision = ColorToolkitCLI.run(["export", "a.b=red", "a-b=blue"])
    #expect(collision.status == .usage)
    #expect(collision.diagnostic.contains("a-b"))

    #expect(ColorToolkitCLI.run(["export", "a.b=red", "c=blue"]).status == .success)
    #expect(ColorToolkitCLI.run(["export", "red", "red"]).status == .success)
  }
  ...
  func failuresNameTheirEntry() {
    ...
    let outcome = ColorToolkitCLI.run(["export", "ok=red", "bad=notacolor"])
  ...
  func exportDefaultsToADocument() {
    #expect(ColorToolkitCLI.run(["export"]).status == .usage)
    ...
    #expect(ColorToolkitCLI.run(["export", "red"]).output.hasPrefix(":root {"))
  }
}
...
struct TokensCommandTests {
  ...
  func aTokenFileImports() {
    ...
    Self.withFile(Self.document) { path in
      let outcome = ColorToolkitCLI.run(["tokens", path])
      #expect(outcome.status == .success)
  ...
  func theListingKeepsTheAuthoredSpelling() {
    ...
    Self.withFile(Self.document) { path in
      ...
      let shaped = ColorToolkitCLI.run(["tokens", path, "--shape",
  ...
  func fileFailuresAreFailuresNotMisuse() {
    ...
    let missing = ColorToolkitCLI.run(["tokens", "/nowhere/at/all.json"])
